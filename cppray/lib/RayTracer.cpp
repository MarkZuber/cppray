...
{
  shared_ptr<IntersectionInfo> info = TestIntersection(ray, nullptr);
  if (info->IsHit())
  {
...
shared_ptr<IntersectionInfo> SimpleRayTracer::TestIntersection(
    const Ray &ray, const shared_ptr<Shape> &exclude)
{
  int hitCount = 0;
  shared_ptr<IntersectionInfo> best = make_shared<IntersectionInfo>();
  best->SetDistance(std::numeric_limits<double>::max());

  for (auto &shape : _scene->Shapes())
  {
    if (!exclude || shape->Id() != exclude->Id())
    {
      shared_ptr<IntersectionInfo> info = shape->Intersect(ray);
...
COLOR_VECTOR SimpleRayTracer::RayTrace(shared_ptr<IntersectionInfo> &info, const Ray &ray,
                                       int depth)
{
    ...
    if (depth < _renderData->TraceDepth())
    {
          ...
          info->Element()->GetMaterial()->Reflection() > 0.0)
      {
        Ray reflectionRay = GetReflectionRay(info->Position(), info->Normal(),
                                             ray.Direction());
        shared_ptr<IntersectionInfo> refl =
          ...
          info->Element()->GetMaterial()->Transparency() > 0.0)
      {
        ...
        shared_ptr<IntersectionInfo> refr = info->Element()->Intersect(refractionRay);
        if (refr->IsHit())
        {
      ...
      shared_ptr<IntersectionInfo> shadow;
